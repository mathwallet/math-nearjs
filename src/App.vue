...
export default {
  ...
  methods: {
    async sendTransaction() {
      const provider = new nearApi.providers.JsonRpcProvider("https://rpc.betanet.nearprotocol.com");

      // Account
      const accountId = this.account.accountId;
      const publicKey = this.account.publicKey;
      const receiverId = "math.betanet";

      // RPC
      const accessKeyResponse = await provider.query(`access_key/${accountId}/${publicKey}`,"");
      
      const blockHash = nearApi.utils.serialize.base_decode(accessKeyResponse.block_hash);
      const nonce = accessKeyResponse.nonce;
      
      // Transaction
      const actions = [
          nearApi.transactions.transfer(new BN("1000000000000000000000000")),   // Transfer(1 NEAR)
      ...
      // Send
      const executionOutcome = await provider.sendTransaction(signedTransaction);
